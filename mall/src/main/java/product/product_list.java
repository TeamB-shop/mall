	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(search_key == null || search_word == null) {
				result = new product_query().select_product_page(Integer.parseInt(pageno), this.ctn_per_page);	
			}
			else {
				result = new product_query().select_product_search(search_key, search_word, this.ctn_per_page);	
			}	
		}
		catch (Exception e) {
